...
func new_game() -> void:
	...
	$%Health_Bar.max_value = player.fighter_component.max_hp
	$%Health_Bar.value = player.fighter_component.max_hp
	%CanvasLayer.show()
...
func _physics_process(_delta: float) -> void:
	...
	if action:
		...
		if action.perform():
			_handle_enemy_turns()
			map.update_fov(player.grid_position)
			$%Health_Bar.update_health(player.fighter_component.hp,player.fighter_component.max_hp)
			$%Score.set_score(player.score)
